...
maxApi.addHandler("parametersByNumber", (userNumber, amp, freq, attack, decay, sustain, waveform) => {
    ...
    if (index >= 0 && index < socketIds.length) {
        ...
        console.log(`Bulk parameters updated for user ${userNumber} (${socketId})`);
        maxApi.outlet("parametersSet", { userNumber, amp, freq, attack, decay, sustain, waveform });
        return true;
    } else {
...
maxApi.addHandler("setWaveform", (waveform, clientId) => {
    ...
    updateClientListOutlet();
    console.log(`Waveform set to ${waveform} ${clientId ? `for client ${clientId}` : "for all clients"}`);
    return true;
});
...
maxApi.addHandler("setWaveformByNumber", (userNumber, waveform) => {
    ...
    if (index >= 0 && index < socketIds.length) {
        ...
        console.log(`Waveform set to ${waveform} for user ${userNumber} (${socketId})`);
...
maxApi.addHandler("setCustomWaveform", (waveformData, clientId) => {
    ...
    updateClientListOutlet();
    console.log(`Custom waveform set ${clientId ? `for client ${clientId}` : "for all clients"}`);
    return true;
});
...
io.on("connection", (socket) => {
    ...
    if (debugMode) {
        ...
        sessionId = socket.handshake.headers.cookie?.split(";")
            .find(c => c.trim().startsWith("oscSession="))?.split("=")[1] || uuidv4();
    }
