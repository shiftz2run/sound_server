...
function createOscillatorWithWaveform() {
  ...
  if (currentWaveform === "custom" && customWaveformData) {
    ...
    ["sine", "square", "sawtooth", "triangle"].includes(currentWaveform)
  ) {
    osc.type = currentWaveform;
    console.log("Applied waveform:", currentWaveform);
  } else {
    console.warn(
      `Unknown waveform type: ${currentWaveform}, defaulting to sine`,
    );
    osc.type = "sine";
  }
...
function updateDisplay() {
  console.log(
    `User: ${userId}, Freq: ${frequency.toFixed(1)}, Amp: ${amplitude.toFixed(3)}, Waveform: ${currentWaveform}`,
  );
}
